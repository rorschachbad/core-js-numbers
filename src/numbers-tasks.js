...
function getAverage(value1, value2) {
  ...
  const delta = Math.max(value1, value2) - Math.min(value1, value2);
  return Math.max(value1, value2) - delta / 2;
}
...
function getSumOfDigits(num) {
  const numSeparatedArr = String(num).split('');
  return numSeparatedArr.reduce((acc, el) => acc + +el, 0);
}
/**
